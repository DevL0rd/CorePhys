...
/// CorePhys bases all length units on meters, but you may need different units for your game.
/// You can set this value to use different units. This should be done at application startup
/// and only modified once. Default value is 1.
/// For example, if your game uses pixels for units you can use pixels for all length values
/// sent to CorePhys. There should be no extra cost. However, CorePhys has some internal tolerances
/// and thresholds that have been tuned for meters. By calling this function, CorePhys is able
...
/// confidently use pixels for all the length values sent to CorePhys. All length values returned
/// from CorePhys will also be pixels because CorePhys does not do any scaling internally.
/// However, you are now on the hook for coming up with good values for gravity, density, and
/// forces.
/// @warning This must be modified before any calls to CorePhys
